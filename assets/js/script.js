(function($) {
	...
	// Intro video handling: remove overlay when finished or when skipped
	document.addEventListener('DOMContentLoaded', function(){
		...
		// Initialize progress bars from data-progress attributes and add ARIA
		try{
			var bars = document.querySelectorAll('.progress .progress-bar');
			bars.forEach(function(b){
				var p = b.getAttribute('data-progress') || '0';
				var pct = parseInt(p,10) || 0;
				// set ARIA and role attributes
				...
				try{
					var parent = b.closest('.skills-section') || b.closest('.about-progress') || b.parentNode;
					var label = parent && parent.querySelector('h5');
					if(label && label.textContent){
						b.setAttribute('aria-label', label.textContent.trim() + ' progress');
					}
				}catch(e){}
				// trigger visual width using RAF so CSS transition runs
